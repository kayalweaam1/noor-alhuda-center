...
export default function AssistantDashboard() {
  ...
  const averageRating = notes && notes.length > 0
    ? (notes.filter(n => n.rating).reduce((sum, n) => sum + (n.rating || 0), 0) / notes.filter(n => n.rating).length).toFixed(1)
    : "0";
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
        ...
        <Card className="border-amber-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-4xl font-bold text-amber-900">{averageRating}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-emerald-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-4xl font-bold text-emerald-900">{notes?.length || 0}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-emerald-200">
        <CardHeader>
          <CardTitle className="text-2xl text-emerald-900">الملاحظات والتقييمات</CardTitle>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-1">
                          من: <span className="font-semibold">{note.teacherName || "غير محدد"}</span>
                        </p>
                        {note.rating && (
                          <div className="flex items-center gap-1 mb-2">
                            ...
                            <span className="text-sm text-gray-600 mr-2">({note.rating}/5)</span>
                          </div>
                        )}
                      </div>
